...
from annotation_constants import (detector_bbox_category_id_to_name)  # here id is int
...
def visualize_detector_output(detector_output_path: str,
    ...
    for entry in tqdm(images):
        ...
        label= entry["file"].split('/')[-2]
        annotated_img_path = os.path.join(out_dir, label, f'anno_{image_id}')
...
def main() -> None:
    ...
    parser.add_argument(
        '-cat', '--category', type=int, default=1,
        help='Value could depend on class from 1, 2, 3 for animal, person, vehicle respectively')
    parser.add_argument(
